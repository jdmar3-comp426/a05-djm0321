...
app.post("/app/new", upload.fields([]), (req, res) => {
    var data = {
        user: req.body.user,
...
app.patch("/app/update/user/:id", (req, res) => {
    const stmt = db.prepare(`UPDATE userinfo SET user = COALESCE(?, user), pass = COALESCE(?, pass), email = COALESCE(?, email) WHERE id = ?`).run(req.body.user, md5(req.body.pass), req.body.email, req.params.id);
